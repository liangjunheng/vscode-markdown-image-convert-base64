...
export function readToolboxs(configToolboxs?: []): void {
    ...
    if (languageId == 'markdown' && getConfig("builtin.markdown.activate",Boolean(true))) {
        readToolbox(markdownToolbox);
        readMarkdownToolAction();
    }
    if ((languageId == 'latex' || languageId == 'tex') && getConfig("builtin.latex.activate",Boolean(true))) {
